...
export interface FetchRtvsloOptions {
  feedUrl?: string;
  feedUrls?: string[];
  timeoutMs?: number;
...
export interface FetchRtvsloSuccess {
  ok: true;
  news: NewsCandidate[];
  meta: {
    url: string;
...
export interface FetchRtvsloError {
  ok: false;
  error: {
    message: string;
    url: string;
...
export type FetchRtvsloResult = FetchRtvsloSuccess | FetchRtvsloError;
...
async function fetchSingleFeed(
  ...
  try {
    ...
    console.log(`RTVSLO feed: XML length ${xml.length}`);
...
export async function fetchRtvsloNews(
  options: FetchRtvsloOptions = {}
): Promise<FetchRtvsloResult> {
  const {
    feedUrl,
    feedUrls,
  ...
  console.log(`Fetching ${urls.length} RTVSLO RSS feeds...`);
...
function parseRSSFeed(xml: string): NewsCandidate[] {
  ...
  try {
    ...
    if (!items) {
      console.log("No items found in RTVSLO RSS feed");
      return [];
    }

    console.log(`Found ${items.length} items in RTVSLO RSS feed`);
    ...
    for (const itemXml of items) {
      try {
        ...
        console.error("Failed to parse RTVSLO RSS item:", error);
      }
    }

    console.log(`Parsed ${news.length} news items from ${items.length} RSS items`);
  } catch (error) {
    console.error("Failed to parse RTVSLO RSS feed:", error);
  }

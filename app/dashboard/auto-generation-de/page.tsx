...
export default function AutoGenerationDEPage() {
  ...
  const loadConfig = async () => {
    try {
      const response = await fetch("/api/auto-generation-de/config");
  ...
  const handleTest = async () => {
    ...
    try {
      const response = await fetch("/api/auto-generation-de/generate-now", {
        method: "POST",
      });
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">🇩🇪 Auto Generation (Немецкий канал)</h1>
              <p className="text-gray-600 mt-1">
                Автоматически генерирует и планирует видео из немецких шуток
              </p>
            </div>

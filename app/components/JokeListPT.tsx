...
import { JokeCandidatePT } from "@/lib/ingest-pt/types";
import JokeCardPT from "./JokeCardPT";

interface JokeListItemPT extends JokeCandidatePT {
  _id?: string;
  createdAt?: string;
...
export default function JokeListPT() {
  const [jokes, setJokes] = useState<JokeListItemPT[]>([]);
  ...
  const loadJokes = async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/ingest-pt/queue?limit=100");
      if (!response.ok) {
        throw new Error("[PT] Не удалось загрузить португальские анекдоты");
      }
      const data = await response.json();
      setJokes(data.jokes ?? []);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Произошла ошибка");
      console.error("[PT] Failed to load jokes:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const collectJokes = async () => {
    ...
    try {
      const response = await fetch("/api/ingest-pt/run", {
        method: "POST",
      });
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || "[PT] Не удалось собрать португальские анекдоты");
      }
      ...
      setError(err instanceof Error ? err.message : "Произошла ошибка");
      console.error("[PT] Failed to collect jokes:", err);
    } finally {
      setCollecting(false);
    }
  };
  ...
  const cleanupLongJokes = async () => {
    if (!confirm("Вы уверены? Это пометит все pending португальские анекдоты длиннее 500 символов как deleted.")) {
      return;
    }
    ...
    try {
      const response = await fetch("/api/debug-pt/cleanup-long-jokes", {
        method: "POST",
      });

      if (!response.ok) {
        throw new Error("[PT] Не удалось очистить длинные анекдоты");
      }

      const result = await response.json();
      console.log("[PT] Cleanup result:", result);
      setCleanupResult({
      ...
      setError(err instanceof Error ? err.message : "Произошла ошибка");
      console.error("[PT] Failed to cleanup long jokes:", err);
    } finally {
  ...
  const cleanupLongJokes600 = async () => {
    if (!confirm("Вы уверены? Это пометит все pending португальские анекдоты длиннее 600 символов как deleted.")) {
      return;
    }
    ...
    setCleanupResult(null);
    try {
      const response = await fetch("/api/debug-pt/cleanup-long-jokes-600", {
        method: "POST",
      });
  ...
  const cleanupLongJokes650 = async () => {
    if (!confirm("Вы уверены? Это пометит все pending португальские анекдоты длиннее 650 символов как deleted.")) {
      return;
    }
    ...
    setCleanupResult(null);
    try {
      const response = await fetch("/api/debug-pt/cleanup-long-jokes-650", {
        method: "POST",
      });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-900">
          🇵🇹 Португальские анекдоты ({filteredJokes.length} {showOnlyPending && `из ${jokes.length}`})
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredJokes.map((joke) => (
          <JokeCardPT key={joke._id} joke={joke} />
        ))}
      </div>

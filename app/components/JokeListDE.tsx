...
export default function JokeListDE() {
  ...
  const cleanupLongJokes = async () => {
    if (!confirm("Вы уверены? Это пометит все pending немецкие анекдоты длиннее 500 символов как deleted.")) {
      return;
    }
    ...
    try {
      const response = await fetch("/api/debug-de/cleanup-long-jokes", {
        method: "POST",
      });

      if (!response.ok) {
        throw new Error("[DE] Не удалось очистить длинные анекдоты");
      }

      const result = await response.json();
      console.log("[DE] Cleanup result:", result);
      setCleanupResult({
      ...
      setError(err instanceof Error ? err.message : "Произошла ошибка");
      console.error("[DE] Failed to cleanup long jokes:", err);
    } finally {
  ...
  const cleanupLongJokes600 = async () => {
    if (!confirm("Вы уверены? Это пометит все pending немецкие анекдоты длиннее 600 символов как deleted.")) {
      return;
    }
    ...
    setCleanupResult(null);
    try {
      const response = await fetch("/api/debug-de/cleanup-long-jokes-600", {
        method: "POST",
      });
  ...
  const cleanupLongJokes650 = async () => {
    if (!confirm("Вы уверены? Это пометит все pending немецкие анекдоты длиннее 650 символов как deleted.")) {
      return;
    }
    ...
    setCleanupResult(null);
    try {
      const response = await fetch("/api/debug-de/cleanup-long-jokes-650", {
        method: "POST",
      });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-900">
          🇩🇪 Немецкие анекдоты ({filteredJokes.length} {showOnlyPending && `из ${jokes.length}`})
        </h2>
      ...
      {filteredJokes.length === 0 && (
        <div className="text-center py-12 text-gray-500">
          Нет доступных немецких анекдотов. Нажмите &quot;Собрать анекдоты&quot; чтобы загрузить.
        </div>
      )}
    </div>
  );

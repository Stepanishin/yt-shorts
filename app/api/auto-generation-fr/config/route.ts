...
import {
  ...
  saveAutoGenerationConfigFR,
  deleteAutoGenerationConfigFR,
  AutoGenerationConfigFR,
} from "@/lib/db/auto-generation-fr";
import { getUserByGoogleId } from "@/lib/db/users";

/**
 * GET /api/auto-generation-fr/config
 * Get current auto-generation configuration for French videos
 */
  ...
  try {
    ...
    if (config?.template?.text) {
      ...
      if (needsColorMigration || needsDurationMigration) {
        ...
        await saveAutoGenerationConfigFR(migratedConfig);
      }
    }
    ...
    console.error("Error fetching auto-generation-fr config:", error);
    return NextResponse.json(
...
/**
 * POST /api/auto-generation-fr/config
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingConfig = await getAutoGenerationConfigFR(user._id!.toString());
    ...
    const savedConfig = await saveAutoGenerationConfigFR(configData);
    ...
    console.error("Error saving auto-generation-fr config:", error);
    return NextResponse.json(
...
/**
 * DELETE /api/auto-generation-fr/config
 * Delete auto-generation configuration for French videos
 */
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deleted = await deleteAutoGenerationConfigFR(user._id!.toString());
    ...
    console.error("Error deleting auto-generation-fr config:", error);
    return NextResponse.json(

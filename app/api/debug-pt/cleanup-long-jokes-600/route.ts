...
/**
 * POST /api/debug-pt/cleanup-long-jokes-600
 * Помечает все pending немецкие анекдоты длиннее 600 символов как deleted
 */
export async function POST() {
  try {
    const collection = await getJokeCandidateCollectionPT();

    // Находим все pending анекдоты длиннее 600
    ...
    console.log(`[PT] Found ${longJokes.length} pending jokes longer than 600 characters`);
    ...
    console.log(`[PT] Marked ${result.modifiedCount} jokes as deleted`);
    ...
    console.error("[PT] Error cleaning up long jokes:", error);
    return NextResponse.json(
      {
